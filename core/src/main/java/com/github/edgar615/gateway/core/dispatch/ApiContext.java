...
import com.github.edgar615.gateway.core.utils.NumberUtils;
...
public interface ApiContext {
     ...
     * @param target 目标对象
     */
    static void copyProperties(ApiContext source, ApiContext target) {
        ...
        if (source.apiDefinition() != null) {
            //copy会导致性能下降，而API定义基本不会发生变化，所以这里不在copy
//      target.setApiDefinition(source.apiDefinition().copy());
    ...
    default ApiContext copy() {
        ...
        if (body() == null) {
            apiContext =
                    new ApiContextImpl(id(), method(), path(), ArrayListMultimap.create(headers()),
        ...
        ApiContext.copyProperties(this, apiContext);
        return apiContext;
    }

    default JsonObject copyJson(
            JsonObject jsonObject) {//直接使用原生的JsonObject.copy方法，会导致JSON的KEY顺序错乱，校验签名的算法会失效
