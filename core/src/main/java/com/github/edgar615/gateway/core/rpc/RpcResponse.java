...
public interface RpcResponse {
        ...
        if (data.startsWith("{") && data.endsWith("}")) {
            try {
                ...
                throw SystemException.wrap(DefaultErrorCode.INVALID_JSON, e)
                        .set("details",
                                "The result of RPC is incorrect, It should be a JSON object");
            }
        }
        if (data.startsWith("[") && data.endsWith("]")) {
            try {
    ...
    default RpcResponse copy() {
        if (isArray()) {
            return RpcResponse
                    .createJsonArray(id(), statusCode(), responseArray().copy(), elapsedTime());
        }
        return RpcResponse
                .createJsonObject(id(), statusCode(), responseObject().copy(), elapsedTime());
    }
