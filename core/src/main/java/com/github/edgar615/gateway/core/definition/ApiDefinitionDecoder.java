...
class ApiDefinitionDecoder implements Function<JsonObject, ApiDefinition> {
    ...
    private ApiDefinition createApiDefinition(JsonObject jsonObject) {
        ...
        if (jsonObject.getValue("type") instanceof String
            ...
            AntPathApiDefinition antPathApiDefinition =
                    (AntPathApiDefinition) ApiDefinition.antStyle(name, method, path,
            ...
            return ApiDefinition.regexStyle(name, method, path,
    ...
    private List<Endpoint> createEndpoints(JsonArray jsonArray) {
        ...
        //只能有名称为default的endpoint
        long defaultCount = endpoints.stream().filter(e -> e.name().equals("default")).count();
        if (defaultCount > 1) {
            throw new RuntimeException("duplicate default endpoint");
        }
        return endpoints;
    }
